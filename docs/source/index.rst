Sintetizador de saídas do NEWAVE
=================================

**Versão:** |release|

**Data:** |today|

O *sintetizador-newave* é um pacote Python para formatação dos arquivos de saída do modelo
 `NEWAVE <https://www.cepel.br/linhas-de-pesquisa/newave/>`_. O NEWAVE é
desenvolvido pelo `CEPEL <http://www.cepel.br/>`_ e utilizado para a planejamento da operação do Sistema Interligado Nacional (SIN).

* :ref:`genindex`

.. toctree::
   :caption: Apresentação 
   :maxdepth: 3
   :hidden:

   ./install.rst

.. toctree::
   :caption: Referência 
   :maxdepth: 3
   :hidden:

   reference/sintetizador-newave/index.rst

.. toctree::
   :caption: Exemplos 
   :maxdepth: 3
   :hidden:

   examples/index.rst
